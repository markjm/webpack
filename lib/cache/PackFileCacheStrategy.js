...
class Pack {
	constructor(logger, maxAge, _isNewlyCreated = false) {
		...
		this.logger = logger;
		this.maxAge = maxAge;
		this._isNewlyCreated = _isNewlyCreated;
	}
	...
	set(identifier, etag, data) {
		if (!this.invalid) {
			...
			this.logger.log(
				`Pack got ${
					this._isNewlyCreated ? "created" : "invalid"
				} because of write to: ${identifier}`
			);
		}
		const info = this.itemInfo.get(identifier);
		if (info === undefined) {
	...
	_openPack() {
		...
		return this.fileSerializer
			...
			.then(pack => {
				...
				return new Pack(logger, this.maxAge, /* _isNewlyCreated */ true);
			})
			.catch(err => {
				this.logger.warn(
